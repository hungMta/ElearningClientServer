module.exports = Object.freeze({
    ...
    SELECT_ALL_COURSE_CLIENT: ' select my_course.idcourse,my_course.name,my_course.image,    my_course.description, my_course.number,coalesce( my_enroll.is_enroll,0) is_enroll from (select course.idcourse, course.name, course.image,course.description ,coalesce(t1.number,0) number  from (select count(iduser) as number  ,idcourse from enroll group by idcourse) t1  right join course on course.idcourse = t1.idcourse  where t1.idcourse = 1) my_course left join (select  iduser,idcourse, count(idenroll) is_enroll from enroll where iduser = 1 and idcourse = 1) my_enroll on my_course.idcourse = my_enroll.idcourse',
